...
export default function CorrectionDataSheet({
  ...
  const { clearAllEdits, editedCount, edits, importEdits } = useEdits()
  ...
  const importFile = async (file?: File) => {
    ...
    try {
      ...
      if (result.imported === 0) {
        ...
        setMessage(`${result.imported} Korrekturen importiert${result.skipped ? `, ${result.skipped} Einträge übersprungen` : ''}.`)
      }
    } catch {
    ...
    <>
      ...
      <aside className={`correction-sheet${open ? ' is-open' : ''}`} aria-label="Lokale Korrekturdaten" aria-hidden={!open}>
        ...
        <div className="correction-summary">
          ...
          <span>{editedCount === 1 ? 'lokale Korrektur auf diesem Gerät' : 'lokale Korrekturen auf diesem Gerät'}</span>
        </div>

        <section className="correction-section">
          <h3>Export</h3>
          <p>Speichert alle lokalen Korrekturen inklusive Lebensstatus als JSON-Datei. Die Datei kann persönliche Daten enthalten.</p>
        ...
        <section className="correction-section">
          <h3>Import</h3>
          <p>Importierte Korrekturen werden mit den vorhandenen lokalen Änderungen zusammengeführt. Unbekannte Personen-IDs werden ignoriert.</p>
          <input
            ref={inputRef}
