...
function TreePartnerCard({
  ...
  const { mode } = usePrivacy()
  const { getPartnerLifeStatus, getPartnerMember, hasPartnerEdit } = useEdits()
  ...
  return (
    <button
      ...
      onClick={() => onOpen(effectiveMember)}
      aria-label={`${effectiveMember.name}, ${relationLabel(effectiveMember)} – Details öffnen`}
    >
      <span className="tree-partner-type">{relationLabel(effectiveMember)}</span>
      ...
      <span className={`tree-partner-life${protectedMember ? ' protected-value' : ''}`}>
        {protectedMember
          ...
          : effectiveMember.birth
            ? `* ${formatDate(effectiveMember.birth)}`
            : effectiveMember.birthPlace ?? 'Lebensdaten offen'}
      </span>
...
function DetailPanel({
  ...
  const { isFavorite, toggleFavorite } = useFamilyNavigation()
        ...
        <section className="detail-section">
          ...
          <div className="relationship-list">
            {person.partners.map((partner, index) => {
              const partnerId = partnerMemberId(person.id, index)
              const effectivePartner = getPartnerMember(partnerId)
              ...
              return (
                <article className="relationship-card" key={partnerId}>
                  ...
                  {protectedPartner ? (
                    ...
                    <>
                      {(shownPartner.birth || shownPartner.birthPlace) && (
                        <span>
                          Geboren {shownPartner.birth ? formatDate(shownPartner.birth) : 'Datum unbekannt'}
                          {shownPartner.birthPlace ? ` · ${shownPartner.birthPlace}` : ''}
                        </span>
                      )}
                      {(shownPartner.death || shownPartner.deathPlace) && (
                        <span>
                          Gestorben {shownPartner.death ? formatDate(shownPartner.death) : 'Datum unbekannt'}
                          {shownPartner.deathPlace ? ` · ${shownPartner.deathPlace}` : ''}
                        </span>
                      )}
                    </>
                  )}
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-layout">
        <section className={`tree-area${viewMode === 'home' ? ' is-home' : ''}`}>
          {viewMode !== 'home' && (
